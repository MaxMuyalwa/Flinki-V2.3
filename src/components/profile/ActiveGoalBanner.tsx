            ...
            <div className="flex items-end justify-between">
              <div className="space-y-1 sm:space-y-2">
                ...
                <Link to={`/explore`} className="block group">
                  <h3 className="text-xl sm:text-2xl font-bold leading-tight text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] group-hover:text-orange-500 transition-colors flex items-center gap-2">
              ...
              <Link
                to="/explore"
                className="flex items-center gap-1.5 sm:gap-2 rounded-full bg-orange-500 px-3 py-1.5 sm:px-4 sm:py-2 text-[10px] sm:text-xs font-bold uppercase tracking-wider text-white shadow-lg transition-all hover:bg-white hover:text-orange-500 active:scale-95"
              >
        ...
        {CHALLENGES.map((_, idx) => (
          <button
            ...
            className={`h-1.5 rounded-full transition-all ${
              idx === currentIndex ? 'w-6 bg-orange-500' : 'w-1.5 bg-white/50 hover:bg-white/80'
            }`}
          />
        ))}
      </div>
